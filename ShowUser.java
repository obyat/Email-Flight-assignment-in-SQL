...
while (rest.next()) {
  System.out.println(rest.getInt("eid") + "\n");
}
...
*/
		...
		try {
			...
			System.out
					.println("\n" +"Please enter the email to be matched:");
			email = readEntry(inn, "email: ");
		} catch (IOException e) {
			System.out.println("Problem with user input, please try again\n");
			System.exit(1);
		}
...
String query = "select" + " firstname, lastname, email from userdb where email="
 + "'" + email + "'";
			try {
...
+ "," + " " + "Lastname: " + rset.getString("lastname") + "," + " " + "Email: " 
