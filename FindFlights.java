...
try {
			...
			origin = readEntry(in1, "origin city: ");
		} catch (IOException e) {
			System.out.println("Problem with user input, please try again\n");
			System.exit(1);
		}
	...
	try {
			...
			destination = readEntry(in2, "destination city: ");
		} catch (IOException e) {
			System.out.println("Problem with user input, please try again\n");
			System.exit(1);
		}	
	...
	public static boolean assignFlight() throws SQLException {
PreparedStatement p = null;
		ResultSet rset = null;
    String origin = null;
    String destination = null;

String query = "select" + " f.origin, f.destination, f.distance, e.ename, a.aname, a.cruisingrange, e.salary from flights f, aircraft a, certified c, employees e1, employees e where a.aid=c.aid and e.eid=c.eid and a.cruisingrange > f.distance and origin like "
 + "'" + "%" + origin + "%"  + "'" + " and destination like "
 + "'" + "%" + destination + "%"  + "'" + " and e.salary < e1.salary  order by e.salary offset 0 rows fetch next 1 rows only";
        ...
        while (rset.next()) {
        System.out.println("\n" + "Flight: from " + origin 
+ " to " + destination + "," + " " + "distance: " + rset.getString("distance"));

        System.out.println("Pilot chosen: " + rset.getString("ename") 
+ ", " + "Salary " + rset.getString("salary"));


        System.out.println("Aircraft chosen: " + rset.getString("aname") 
+ ", " + "Cruisingrange " + rset.getString("cruisingrange"));
			}
